//const h2 = document.createElement("h2");
//h2.textContent = "content added by JavaScript file";
//document.querySelector("body").appendChild(h2);

//make background-color of images opposing color when playing

//check if other song is playing and turn off if so when play pressed


let firstSongImage = document.getElementById('song1-pic');
let secondSongImage = document.getElementById('song2-pic');
let thirdSongImage = document.getElementById('song3-pic');
let fourthSongImage = document.getElementById('song4-pic');

//animate image when mouse is on audio controls
function shrinkPadding() {

}